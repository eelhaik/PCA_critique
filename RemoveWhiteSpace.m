...
boundingbox_hori(1) = boundingbox_hori(1) -1; %modified by changeing + to -
boundingbox_vert(1) = boundingbox_vert(1) -1; %modified by changeing + to -
...
% Modified by adding 5
u_out = u_in(boundingbox_vert(1) : boundingbox_vert(2)+5, boundingbox_hori(1) : boundingbox_hori(2)+5, :);

%
if (it_is_a_file_flag)
    % 	imwrite(u_out, output_filename); %Original command
%     disp('Saving as tif'); imwrite(u_out, output_filename,'tif','Resolution', 600); 
    disp('Saving as png'); imwrite(u_out, output_filename,'png'); 
end
